...
static freertos_message_queue::FreeRTOSMessageQueue<motor_messages::GearMotorMove> motor_queue_left("Motor Queue Left");
static freertos_message_queue::FreeRTOSMessageQueue<motor_messages::GearMotorMove> motor_queue_right("Motor Queue Right");
...
static motor_handler::MotorInterruptHandler motor_interrupt_right(
    motor_queue_right, head_gear_tasks::get_right_gear_queues(), motor_hardware_right);
...
static motor_handler::MotorInterruptHandler motor_interrupt_left(
    motor_queue_left, head_gear_tasks::get_left_gear_queues(), motor_hardware_left);
...
auto main() -> int {
    ...
    head_sensor_tasks::start_tasks(*central_tasks::get_tasks().can_writer,
                              head_peripheral_tasks::get_i2c3_client(),
                              head_peripheral_tasks::get_i2c3_poller_client(),
                              id, eeprom_hardware_iface);

    initialize_gear_timer(gear_callback_wrapper, left_enc_overflow_callback_glue, right_enc_overflow_callback_glue);
    head_gear_tasks::start_tasks(
        *central_tasks::get_tasks().can_writer,
        id, motor_left,
        motor_right, head_peripheral_tasks::get_spi2_client(),
