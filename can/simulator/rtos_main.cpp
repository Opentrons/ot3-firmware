...
// Parser listeners
static auto a_builder = DispatchParseTarget<HandlerA, GetSpeedRequest>(a_handler);
static auto b_builder = DispatchParseTarget<HandlerB, StopRequest>(b_handler);

// Buffer poller functions
static auto poller_a = freertos_can_dispatch::FreeRTOSCanBufferPoller{a_buffer, a_builder};
static auto poller_b = freertos_can_dispatch::FreeRTOSCanBufferPoller{b_buffer, b_builder};

// Buffer poller tasks
static auto dispatcher_task_a = FreeRTOSTask<256, 5, decltype(poller_a)>("dispatcher_a", poller_a);
static auto dispatcher_task_b = FreeRTOSTask<256, 5, decltype(poller_b)>("dispatcher_b", poller_b);

static auto a_target = DispatchBufferTarget<BufferType, GetSpeedRequest>{a_buffer};
...
static auto poller = freertos_can_dispatch::FreeRTOSCanBufferPoller(buffer, dispatcher);
static auto dispatcher_task = FreeRTOSTask<256, 5, decltype(poller)>("dispatcher", poller);
