...
SCENARIO("Testing a move group") {
    auto group = move_group::MoveGroup<5, can::messages::AddLinearMoveRequest,
    ...
    GIVEN("a move group") {
        ...
        WHEN("set_move is called") {
            ...
            THEN("the stored type is correct.") {
                REQUIRE(
                    std::holds_alternative<can::messages::AddLinearMoveRequest>(
                        read_group));
            }
            THEN("the object is stored correctly.") {
                REQUIRE(std::get<can::messages::AddLinearMoveRequest>(
        ...
        WHEN("a homing request is sent with set_move") {
            auto home_request = can::messages::HomeRequest{
            ...
            THEN("the stored type is correct.") {
                REQUIRE(std::holds_alternative<can::messages::HomeRequest>(
                    get_group));
            }
            THEN("the object is stored correctly.") {
                REQUIRE(std::get<can::messages::HomeRequest>(get_group) ==
                        home_request);
            }
            THEN("it is not empty") { REQUIRE(!group.empty()); }
            THEN("the size is 1") { REQUIRE(group.size() == 1); }
