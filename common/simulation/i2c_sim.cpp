...
auto sim_i2c::SimI2C::central_transmit(uint8_t *data, uint16_t size,
    ...
    LOG("Received a register bit: %d", static_cast<int>(reg));
    ...
    if (write_bit == 1) {
        ...
        LOG("Storing %d in register %d", static_cast<int>(store_in_register),
            static_cast<int>(reg));
...
auto sim_i2c::SimI2C::central_receive(uint8_t *data, uint16_t size,
    ...
    LOG("Grabbing data %d from register %d", static_cast<int>(data_from_reg),
        static_cast<int>(reg));
    auto *iter = data;
