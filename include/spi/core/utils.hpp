...
[[nodiscard]] inline constexpr auto tag_in_token(uint32_t token, ResponseTag tag) -> bool { // utilize
    return bool(token & (1 << (static_cast<size_t>(tag) + static_cast<size_t>(8))));
}

[[nodiscard]] inline constexpr auto build_token(uint8_t reg, uint8_t tags = 0) -> uint32_t {
    return (static_cast<uint32_t>(tags) << 8) | (reg);
}

template <typename RegType>
[[nodiscard]] inline constexpr auto reg_from_token(uint32_t id) -> RegType { // utilize? If not, discard
