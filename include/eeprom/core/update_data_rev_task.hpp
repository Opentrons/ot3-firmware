...
class UpdateDataRevHandler : accessor::ReadListener {
  public:
    UpdateDataRevHandler(EEPromClient& eeprom_client, dev_data::DevDataTailAccessor<EEPromClient>& tail_accessor)
  ...
  private:
    ...
    void visit(const DataTableUpdateMessage& m) {
        ...
        if (m.data_rev == current_data_rev + 1) {
            for (auto& i: m.data_table) {
                // add the new data table entry
                table_creator.create_data_part(i.first, i.second);
                // wait for the table update to finish
                while (!table_creator.ready()) { vTaskDelay(10); }
            }
            std::ignore = bit_utils::int_to_bytes(m.data_rev, data_rev_backing.begin(), data_rev_backing.end());
    ...
    void read_complete(uint32_t message_index) final {
        // we don't send requests for reads to the DevDataAccessor so this will only be called by the data tail
        std::ignore = message_index;

        // test if data is set to 0xFFFF, some chips default to 0x0000 but this is fine since
        // if it's set to that then it's already where we want it to be as a default
        auto delivery_state = std::vector<uint8_t>(
            addresses::data_revision_length, 0xFF);
        ...
        std::ignore = bit_utils::bytes_to_int(data_rev_backing.begin(), data_rev_backing.end(), current_data_rev);
    ...
    data_revision::DataRevisionType data_rev_backing = data_revision::DataRevisionType{};
...
class UpdateDataRevTask {
  public:
     ...
     */
    template <task::TaskClient EEPromClient>
    [[noreturn]] void operator()(EEPromClient* eeprom_client,
