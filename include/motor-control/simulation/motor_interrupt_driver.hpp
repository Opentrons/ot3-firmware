...
class MotorInterruptDriver {
  public:
    MotorInterruptDriver(
        ...
        freertos_message_queue::FreeRTOSMessageQueue>& h): task_entry{q, h}, task("motor interrupt driver", task_entry) {}

  private:
    struct TaskEntry {
        TaskEntry(freertos_message_queue::FreeRTOSMessageQueue<motor_messages::Move>& q, motor_handler::MotorInterruptHandler<
               freertos_message_queue::FreeRTOSMessageQueue>& h) : queue{q}, handler{h} {}
        ...
        void operator()() {
            while (true) {
                ...
                if (queue.peek(&move, portMAX_DELAY)) {
                    LOG("Enabling motor interrupt handler for group %d, seq %d, duration %d\n", move.group_id, move.seq_id, move.duration);
                    do {
