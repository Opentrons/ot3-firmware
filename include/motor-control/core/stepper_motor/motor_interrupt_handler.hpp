    ...
    [[nodiscard]] auto stop_condition_met() {
        ...
        if (buffered_move.check_stop_condition(MoveStopCondition::sync_line) &&
            ...
            if (buffered_move.check_stop_condition(
                ...
                // speeds higher than the discontinuity speed and have a acceleration value.
                // this lets them send a response for when the condition is met,
                // but still continue on the move so it can decelerate safely.
                buffered_move.stop_condition = static_cast<uint8_t>(MoveStopCondition::none);
