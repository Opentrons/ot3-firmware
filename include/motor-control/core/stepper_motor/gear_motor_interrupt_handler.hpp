...
namespace gear_motor_handler {
// TODO this needs to be refactored to share more code with the regular
// stepper motor. We should maybe also not have calibration functions
...
template <template <class> class QueueImpl,
          pipettes::tasks::gear_move_status::TaskClient StatusClient>
requires MessageQueue<QueueImpl<GearMotorMove>, GearMotorMove>
class MotorInterruptHandler {
  public:
    using GenericQueue = QueueImpl<GearMotorMove>;
    ...
    auto tip_sensed() -> bool {
        return hardware.check_tip_sense();
    }

    auto require_tip(GearMotorMove move) -> bool {
        return move.action == can_ids::PipetteTipActionType::pick_up;
    }
    ...
    void finish_current_move(
        ...
        // A tip action motor move is successful if:
        // 1. Is a pick up tip action and tip sensed
        // 2. Is a drop tip action and tip is not sensed
        bool pick_up_success = require_tip(buffered_move) && tip_sensed();
        bool drop_success = !require_tip(buffered_move) && !tip_sensed();
        bool success = pick_up_success || drop_success;
        if (buffered_move.group_id != NO_GROUP) {
            auto ack = GearMotorAck{
                buffered_move.group_id,
                buffered_move.seq_id,
                static_cast<uint32_t>(position_tracker >> 31),
                pulses,
                ack_msg_id,
                success,
                buffered_move.action
            };
            static_cast<void>(
                status_queue_client.send_move_status_reporter_queue(ack));
        }
        set_buffered_move(GearMotorMove{});
    }
    ...
    [[nodiscard]] auto get_buffered_move() const -> GearMotorMove {
        return buffered_move;
    }
    void set_buffered_move(GearMotorMove new_move) { buffered_move = new_move; }
  ...
  private:
    ...
    GearMotorMove buffered_move = GearMotorMove{};
};
