...
SCENARIO("converting data to 24 bit value from IC registers") {
    GIVEN("The max MSB and LSB values of the fdc1004") {
        ...
        int32_t converted_registers =
            sensors::fdc1004_utils::convert_reads(MSB, LSB);
        LOG("the converted value: %d", converted_registers);
        REQUIRE(float(converted_registers) ==
                sensors::fdc1004_utils::MAX_RAW_MEASUREMENT);
        int32_t capacitance = sensors::fdc1004_utils::convert_capacitance(
            converted_registers, 1, 0.0);
    ...
    GIVEN("Half the max MSB and LSB values of the fdc1004") {
        ...
        REQUIRE(converted_registers ==
                (int(sensors::fdc1004_utils::MAX_RAW_MEASUREMENT) >> 1));
        float capacitance = sensors::fdc1004_utils::convert_capacitance(
            converted_registers, 1, 0.0);
        float expected = 7.5;
